window.addEventListener('load', () => {
    ...
    videoElement.setAttribute('muted',''); videoElement.muted = true;
    cameraBox.appendChild(videoElement);

    // Canvas 320x240 — جودة كافية بدون بطء
    ...
    focusedBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        stopAlarm(); playFocusReturnChime();
        detectionHistory = Array(WINDOW_SECS).fill(true);
        framesSeen = framesTotal = 0; lastFocused = true;
    ...
    let totalRounds = 0, totalFocusSeconds = 0, segmentSeconds = 0, segmentDistracts = 0;
    ...
    function setStatusByReason(reason) {
        if      (reason === 'frontal')      setStatus('👁 FOCUSED — بيبص في الشاشة',  '#FAF6F0');
        else if (reason === 'looking_down') setStatus('📖 FOCUSED — بيقرأ أو بيكتب',  '#90EE90');
        else if (reason === 'sideways')     setStatus('👀 تشتت! بيبص يمين أو شمال',   '#FF4444');
        else if (reason === 'phone')        setStatus('📱 تليفون في الإطار! ركّز!',   '#FF6B00');
        else                                setStatus('FOCUSED — Sentinel Watching.',  '#FAF6F0');
    }

    function loadPreset() {
        const p = pomodoroPreset.value.split('-').map(Number);
        WORK_TIME = p[0]*60; BREAK_TIME = p[1]*60;
    }
    ...
    pomodoroPreset.addEventListener('change', () => {
        if (sessionActive) return;
        loadPreset(); if (pomodoroMode) updateTimerDisplay();
    });

    // ─── Audio — مضمون يشتغل على HTTPS ──────────────────────────────────────
    ...
    function playAlarmSound(reason) {
        if (!audioCtx) return;
        audioCtx.resume().then(() => {
            if (reason === 'phone') {
                beep(1200,0.2,0); beep(600,0.2,250); beep(1200,0.2,500);
            } else {
                // إنذار واضح وقوي
                beep(900,0.3,0); beep(700,0.3,350); beep(900,0.3,700);
            }
        });
    }
    ...
    function startAlarm(reason) {
        ...
        playAlarmSound(reason);
        alarmIntervalId = setInterval(() => {
            if (!isAlertActive) { stopAlarm(); return; }
            audioCtx.resume().then(() => beep(880, 0.2));
        }, 800);
    }
    ...
    function playFocusReturnChime() {
        if (!audioCtx) return;
        audioCtx.resume().then(() => {
            beep(523,0.15,0); beep(659,0.15,160); beep(784,0.25,320);
        });
    }
    function playPhaseChime() {
        if (!audioCtx) return;
        audioCtx.resume().then(() => { beep(600,0.3,0); beep(900,0.3,300); });
    }
    ...
    async function checkFaceWithBackend() {
        ...
        // لو في request شغّال — استخدم آخر نتيجة معروفة
        if (isFetching) {
            framesTotal++;
    ...
    function switchPhase() {
        ...
        if (pomodoroPhase === 'work') {
            ...
            pomodoroLabel.innerText = '☕ BREAK'; pomodoroLabel.style.color = '#90EE90';
            setStatus('☕ Break time — Relax!', '#90EE90');
            if (isAlertActive) stopAlarm();
            cameraBox.style.outline = 'none';
        } else {
            pomodoroPhase = 'work'; pomodoroLeft = WORK_TIME; totalRounds++;
            statRounds.innerText = totalRounds;
            pomodoroLabel.innerText = '💪 WORK'; pomodoroLabel.style.color = 'var(--text-main)';
            setStatus('FOCUSED — Sentinel Watching.', '#FAF6F0');
            resetDetection();
        }
        timerElement.innerText = formatMM(pomodoroLeft);
    ...
    function resetDetection() {
        ...
        isFetching = false; lastFocused = true; lastReason = '';
    }
    ...
    pauseBtn.addEventListener('click', () => {
        ...
        if (sessionPaused) {
            if (isAlertActive) stopAlarm(); resetDetection();
            cameraBox.style.outline = 'none';
            setStatus('⏸ Paused — Session on hold.', '#FFD580');
            pauseBtn.innerText = 'RESUME';
        } else {
            resetDetection(); setStatus('Sentinel: Watching...', '#FAF6F0');
    ...
    startBtn.addEventListener('click', () => {
        if (!sessionActive) {
            ...
            pomodoroToggle.disabled = false; pomodoroPreset.disabled = !pomodoroMode;
            cameraBox.style.outline = 'none';
            updateTimerDisplay();
